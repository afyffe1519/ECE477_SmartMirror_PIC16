...
int wireReadDataBlock(   uint8_t reg, uint8_t *val, unsigned int len)
{
 ...
 /*
    ...
    for(j= 0; j < len ; j++)
    {
      ...
      I2C_Send_NACK(); // communication is finished
     }
    */
  
    i2c2_driver_start();
    i2c2_driver_setAddr((APDS9960_I2C_ADDR << 1));
...
int wireWriteDataByte(unsigned char reg, unsigned char val)
{
 /*
    ...
    */
    ...
    i2c2_driver_start();
    i2c2_driver_setAddr(APDS9960_I2C_ADDR);
    i2c2_driver_setAddr(reg);
